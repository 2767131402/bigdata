 ...
 * @author oracle
 * 分布式锁
 * 有十个客户端：在zk创建十临时znode,只保证最小（大）id获取锁
 * 把zk下的所有临时目录，加list,对list排序
 *
 */
public class DistributedLock implements Watcher{
    ...
    private static final String CONNECTION_STRING = "192.168.6.201:2181";    
    ...
    public boolean createPath( String path, String data, boolean needWatch) throws KeeperException, InterruptedException {
        if(zk.exists(path, needWatch)==null){
            LOG.info( LOG_PREFIX_OF_THREAD + "节点创建成b功, Path: "
                    + this.zk.create( path,
                    data.getBytes(),
    ...
    */
    ...
    public void process(WatchedEvent event) {
        ...
        System.out.println("xxxxxxxxxxxxxxxxxxxxxx:"+this);
    }
}